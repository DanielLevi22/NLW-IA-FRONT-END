...
import { getFFmeg } from "@/lib/ffmpeg";
...
export function VideoInputForm({ onVideoUploaded }:VideoInputFormProps) {
  ...
  async function covertVideoToAudio(video: File) {
    console.log('convert started');

    const ffmpeg = await getFFmeg();
  ...
  async function handleUploadVideo(event: FormEvent<HTMLFormElement>) {
    ...
    const audioFile = await covertVideoToAudio(videoFile);
    ...
    <form className="space-y-6" onSubmit={handleUploadVideo}>
      ...
      <div className="space-y-2">
        ...
        <Textarea
          ...
          placeholder="Inclua palavras-chave mencionadas no video separadas pro virgula (,)" />
      </div>
      <Button 
